...
image::image(QWidget* parent)
    : QWidget(parent), modified(false), whiteBackground(true), myWidth(2),
      scaleFactor(1), primaryColor(Qt::black), secondaryColor(Qt::white),
      has_image(false), crop(false),
      img(QImage(1240, 680, QImage::Format_RGB32)) {
  img.fill(QColor(255, 255, 255));

  allTools.insert(
      std::pair<QString, Tool*>("pencil", new Pencil(&primaryColor, 2, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "eraser", new Eraser(&secondaryColor, 6, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "colorpicker", new ColorPicker(&primaryColor, &secondaryColor, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "brush", new Brush(&primaryColor, myWidth, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "line", new Line(&primaryColor, myWidth, &img)));
  allTools.insert(
      std::pair<QString, Tool*>("bucket", new Bucket(&primaryColor, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "ellipse", new Ellipse(&primaryColor, myWidth, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "rect", new Rect(&primaryColor, myWidth, &img)));
  allTools.insert(std::pair<QString, Tool*>(
      "triangle", new Triangle(&primaryColor, myWidth, &img)));
...
void image::setBrushWidth() {
  ...
  int newWidth = QInputDialog::getInt(
      this, tr("Width"), tr("Select pen width:"), penWidth(), 1, 50, 1, &okay);
  /* Change the width */
  if(okay) {
...
void image::mousePressEvent(QMouseEvent* event) {
  event->setLocalPos(
      QPoint(event->pos().rx() / scaleFactor, event->pos().y() / scaleFactor));
  modified = true;
...
void image::paintEvent(QPaintEvent* event) {
  ...
  painter.drawImage(dirtyRect,
                    img.scaled(img.size().rwidth() * scaleFactor,
                               img.size().rheight() * scaleFactor),
                    dirtyRect);
}
...
void image::resizeCurrentImg() {
  /*QString sizesStr = QInputDialog::getText(this,
                                           "Set new dimensions",
                                           tr("First enter one number for new
     width, then one for new height:")
                                           );
  */
  bool okayW;
  int newWidth = QInputDialog::getInt(this, tr("Width"), tr("Select width:"),
                                      200, 1, 2000, 1, &okayW);
  bool okayH;
  int newHeight = QInputDialog::getInt(this, tr("Height"), tr("Select height:"),
                                       200, 1, 2000, 1, &okayH);
